package unics.okdroid.widget.coordinatorlayout
    ...
    override fun outAppBarOffsetLimit(scale: Float) {
        ...
        titleBarTranslateWidgets.forEach {
            if (it.visibility != View.INVISIBLE) {
                var outAnim = (it.getTag(R.id.ucs_id_extra_coordinator_effect_out_anim) as? Animation)
                if (outAnim == null) {
                    outAnim = createTransOutAnim()
                    it.setTag(R.id.ucs_id_extra_coordinator_effect_out_anim, outAnim)
                }
                outAnim.reset()
                it.clearAnimation()
    ...
    override fun inAppBarOffsetLimit(scale: Float) {
        ...
        titleBarTranslateWidgets.forEach {
            if (it.visibility != View.VISIBLE) {
                ...
                var inAnim = (it.getTag(R.id.ucs_id_extra_coordinator_effect_in_anim) as? Animation)
                if (inAnim == null) {
                    inAnim = createTransInAnim()
                    it.setTag(R.id.ucs_id_extra_coordinator_effect_in_anim, inAnim)
                }
                inAnim.reset()
                it.clearAnimation()

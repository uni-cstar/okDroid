...
/**
 * 通过反射设置：
 * 反射[android.os.SystemProperties]的set方法设置属性
 */
@Deprecated("未测试过")
@SuppressLint("PrivateApi")
fun setSystemPropertyByReflect(propName: String, value: String) {
    val signature = "setSystemPropertyByReflect(propName=$propName value=$value)"
    try {
        Logger.d(TAG, signature)
        val cls = Class.forName("android.os.SystemProperties")
        val method = cls.getMethod("set", String::class.java, String::class.java)
        method.invoke(cls, propName, value)
    } catch (e: Throwable) {
        e.printStackTrace()
        Logger.e(TAG, "$signature failed", e)
    }
}

...
fun getTopPackageName(context: Context): String? {
    return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
        TopPackageNameReaderApi22().get(context)
    } else {
        TopPackageNameReaderDefault().get(context)
    }
}
...
fun Context.getTopPackageNameSafely(): String? {
    val manager = applicationContext.getSystemService(Context.ACTIVITY_SERVICE) as? ActivityManager
        ?: return null
    return if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
        manager.getRunningTasks(1).getOrNull(0)?.topActivity?.packageName
    } else {
        val appInfo = manager.runningAppProcesses?.getOrNull(0)
        if (appInfo != null && appInfo.processName == appInfo.pkgList.getOrNull(0)) {
            appInfo.processName
        } else {
            if (TopPackageNameReaderApi22.canUsageStats(this)) {
                getTopPackageName(this)
            } else {
                //返回一个不准的结果
                manager.getRunningTasks(1).getOrNull(0)?.topActivity?.packageName
            }

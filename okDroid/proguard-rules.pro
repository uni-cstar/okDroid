...
# Gson只需要保持对应的Model类即可 Application classes that will be serialized/deserialized over Gson
...
##---------------Moshi Proguard 参考来源：https://github.com/square/moshi#r8--proguard
# 包含其自身内部结构所需的最低限度规则，无需消费者嵌入自己的规则。但是，如果您使用反射序列化和 R8 或混淆器，则必须在混淆器配置文件中为反射序列化类添加保留规则。